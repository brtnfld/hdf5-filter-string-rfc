...
\noindent\colorbox{yellow!25}{\parbox{\dimexpr\linewidth-2\fboxsep}{%
  ...
  \begin{enumerate}
    \item \textbf{Struct embedding.} Embed \texttt{H5Z\_class2\_t} as the first
          ...
          (\texttt{H5Z\_class3\_t \{ H5Z\_class2\_t base; set\_config\_t set\_config;
          get\_config\_t get\_config; \}}).
...
of a key-value string. For some filters (e.g., ZFP), the mapping from
configuration string to \texttt{cd\_values} is not injective: distinct string
representations may collapse to the same \texttt{cd\_values}, and the original
string cannot be recovered. In the general case, \emph{exact round-tripping of
the parameter string is not achievable without changing what is stored
on-disk}---either by modifying the \texttt{cd\_values} encoding used by the filter
or by changing the DCPL encoding for filter entries to retain the original
string alongside the \texttt{cd\_values}. Consumers MUST NOT rely on exact string
